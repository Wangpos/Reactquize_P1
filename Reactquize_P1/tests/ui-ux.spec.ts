...
test.describe("UI/UX Tests", () => {
  ...
  test("TC011: Visual Feedback", async ({ page }) => {
    ...
    const newFirstOption = page
      ...
      .first();
    await expect(newFirstOption).not.toHaveClass(/bg-green-100|bg-red-100/);
  ...
  test("TC011b: Timer Visual Feedback", async ({ page }) => {
    ...
    // Verify timer is visible and shows correct format
    const timer = page.locator('[data-testid="timer"]');
    await expect(timer).toBeVisible();
    await expect(timer).toContainText(/\d+s/); // Should show format like "30s"

    // Wait a few seconds and verify timer continues counting down
    await page.waitForTimeout(3000);
    const timerText = await timer.textContent();
    const currentTime = parseInt(timerText?.match(/\d+/)?.[0] || "30");
    expect(currentTime).toBeLessThan(30);

    // Currently the app doesn't have warning states, so we just verify basic functionality
  });
});
